...
//	        }
	...
	public GrafoIterator<T> createBFSIterator(Vertice<T> verticeInicial) {
		return new GrafoIteratorBFS(verticeInicial);
	}

	public GrafoIterator<T> createDFSIterator(Vertice<T> verticeInicial) {
		return new GrafoIteratorDFS(verticeInicial);
	}

	private class GrafoIteratorBFS implements GrafoIterator<T> {

		private Queue<Vertice<T>> fila;
		private boolean[] indicesVisitados;

		public GrafoIteratorBFS(Vertice<T> verticeInicial) {
			fila = new LinkedList<>();
		...
		public Vertice<T> getNext() {
			...
			List<Vertice<T>> verticesVizinhos = incidentes(verticeAtual);
			for(Vertice<T> vizinho: verticesVizinhos) {
